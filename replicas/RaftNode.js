 ...
 *
 * Stage 1 correctness properties (unchanged): persistence-before-ack,
 * current-term commit rule, no-op-on-election, per-peer nextIndex/matchIndex,
 * safe commitIndex clamping, ReadIndex.
 *
 * Stage 2 additions:
 *   • submit() has a fast-path dedupe check (avoids unnecessary log
 *     appends for retried strokes)
 *   • _maybeSnapshot() triggers compaction once applied entries exceed
 *     `snapshotThreshold`; snapshot lives in SQLite, log tail below the
 *     snapshot's lastIncludedIndex gets truncated
 *   • _sendAppendEntriesTo() falls back to InstallSnapshot when a
 *     follower's nextIndex is below our compacted log's firstIndex
 *   • handleInstallSnapshot() reconstitutes a lagging follower from the
 *     leader's snapshot blob
 */
...
class RaftNode extends EventEmitter {
  constructor({ replicaId, peers, persistence, transport, stateMachine, logger, config }) {
    ...
    this.snapshotCount = 0;   // observability: how many snapshots taken here
  }
  ...
  async _sendInstallSnapshotTo(peer) {
    const snap = this.persistence.loadLatestSnapshot();
    if (!snap) return;   // shouldn't happen — nextIndex only drops below firstIndex after we compacted
    const req = {
      term:              this.currentTerm,
      leaderId:          this.replicaId,
      lastIncludedIndex: snap.lastIncludedIndex,
    ...
    const body = res.data;
    if (body.term > this.currentTerm) { this._becomeFollower(body.term); return; }
    // Follower is now at lastIncludedIndex; next log entry to send is +1.
  ...
  _maybeAdvanceCommitIndex() {
    ...
    for (let n = lastIdx; n > this.commitIndex; n--) {
      ...
      if (!e || e.term !== this.currentTerm) continue;   // §5.4.2
      let count = 1;
      for (const peer of this.peers) {
  ...
  _applyCommitted() {
    while (this.stateMachine.lastApplied < this.commitIndex) {
      ...
      if (!entry) break;   // gap — waiting on catch-up
  ...
  async readIndex() {
    ...
    // If commit sits inside a snapshot, we know it committed in some
    // prior term but we've promoted it via install-snapshot — safe.
